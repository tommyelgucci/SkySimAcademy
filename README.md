...
explicación al responder, ver "El curso de teoría" más abajo). El quiz del
módulo (el que aprueba/desaprueba y desbloquea misiones) **no se escribe a
mano**: `deriveModuleQuiz` (`src/content/schema.js`) lo arma juntando las
preguntas de todas las lecciones, así no hay dos bancos que mantener
sincronizados.
...
// src/content/modules/principles-of-flight.json
{
  ...
  "lessons": [
    {
      "id": "four-forces",
      // ids únicos en todo el módulo (conviven en el mismo objeto i18n plano)
      "quiz": { "questions": [{ "id": "four-forces-q1", "correct": 0 }, …] }
    },
    …
  ],
  "quiz": { "passScore": 8, "sampleSize": 10 }  // "questions" se deriva, no se escribe aquí
}
```
...
modules.<moduleId>.lessons.<lessonId>.title / .body (párrafos separados por "\n\n")
modules.<moduleId>.lessons.<lessonId>.keyTakeaway / .simTip
...
- **Vuelo libre = archipiélago disperso** (`FREE_ROAM_SCENARIO` en `SceneManager.js`): la misión "Vuelo libre" ignora el escenario elegido en el selector y siempre construye un mundo mucho más grande (límite del mapa de 6 km, frente a ~1 km de los escenarios normales) con los 4 sitios de aterrizaje repartidos a varios kilómetros entre sí — el aeródromo desértico como base de salida en el origen, y la isla costera, el paso de montaña y la plataforma marina esperando en direcciones distintas. Se puede volar sobre mar abierto y "perderse" de verdad, o explorar hasta toparse con otra pista y aterrizar ahí; el evaluador de aproximación (`FlightEvaluator`) y el mini-mapa ya soportan varias pistas a la vez, así que funciona sin cambios adicionales.
